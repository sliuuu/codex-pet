...
SAFE_WIDTH = min(
    math.floor(CELL_WIDTH * MAX_OCCUPANCY_PERCENT / 100.0),
    CELL_WIDTH - 2 * math.ceil(CELL_WIDTH * MIN_PADDING_PERCENT / 100.0),
)
SAFE_HEIGHT = min(
    math.floor(CELL_HEIGHT * MAX_OCCUPANCY_PERCENT / 100.0),
    CELL_HEIGHT - 2 * math.ceil(CELL_HEIGHT * MIN_PADDING_PERCENT / 100.0),
)
ROW_FRAME_COUNTS = {
    "idle": 6,
    "running-right": 8,
...
def main() -> None:
    ...
    parser.add_argument(
        "--skip-source-safety",
        action="store_true",
        help="Explicitly bypass the pre-extraction source boundary gate.",
    )
    ...
    for state in states:
        ...
        if not args.skip_source_safety:
            from validate_source_strip_safety import validate_strip_image

            with Image.open(strip_path) as opened:
                transparent = remove_chroma_background(opened, chroma_key, args.key_threshold)
            source_report = validate_strip_image(
                transparent,
                ROW_FRAME_COUNTS[state],
                MIN_PADDING_PERCENT,
                MAX_OCCUPANCY_PERCENT,
            )
            if not source_report["ok"]:
                messages = "\n".join(f"- {error}" for error in source_report["errors"])
                raise SystemExit(
                    f"source safety validation failed for {strip_path}; extraction refused:\n{messages}"
                )
        manifest.append(
            extract_state(
                strip_path,
